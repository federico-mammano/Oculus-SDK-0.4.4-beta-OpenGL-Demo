...
void ExampleFeatures1(float * pSpeed, int * pTimesToRenderScene, ovrVector3f * useHmdToEyeViewOffset)
{
    ...
    if (OGL.Key['1']) MakeNewDistortionMeshes(0.001f); // Min eye relief
    if (OGL.Key['2']) MakeNewDistortionMeshes(1.000f); // Max eye relief
    ...
    if (OGL.Key['M'])                         *pSpeed *= 2;    
    if (((clock % 2) != 0) && (OGL.Key['M'])) *pSpeed = 0;
#endif
}
                      ...
                      bool * pClearEyeImage,bool * pUpdateEyeImage)
{
    ...
    if ((OGL.Key['N']) && ((clock % (BLANK_FREQUENCY*2)) == (eye*BLANK_FREQUENCY)))
                      ...
                      float ** pUseYaw, double * pDebugTimeAdjuster,bool * pWaitForGPU)
{
    ...
    if ((OGL.Key['M']) && (((clock % 4) == 0) || ((clock % 4) == 3)) && (eye == 0))   
    {
        *pUseEyePose    = &extraRenderPose;
        *pUseYaw        = &extraYaw;
    ...
    // Non SDK-rendered only.
    if (OGL.Key['4']) *pDebugTimeAdjuster = -0.026; // Greatly underpredicting
    if (OGL.Key['5']) *pDebugTimeAdjuster = -0.006; // Slightly underpredicting
    if (OGL.Key['6']) *pDebugTimeAdjuster = +0.006; // Slightly overpredicting
    if (OGL.Key['7']) *pDebugTimeAdjuster = +0.026; // Greatly overpredicting
